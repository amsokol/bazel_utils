...
load("@bazel_utils_core//internal:workspace_tool.bzl", "workspace_file_label", "workspace_test_tags", "workspace_tool_rule")

_markdownlint_test = workspace_tool_rule(
    tool_attr = "markdownlint",
    tool_default = Label("//:markdownlint-cli2"),
...
def markdownlint_test(
    ...
    Defaults `local = True` and tags `external`, `no-cache`, `no-sandbox`
    (workspace-cd, not hermetic inputs).
    ...
    Args:
      ...
      **kwargs: Forwarded to the test rule (`markdownlint`, `flags`, `size`, …).
    """
    _markdownlint_test(
        name = name,
        workspace = workspace,
