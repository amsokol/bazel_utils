...
Current module version: **0.2.8**. See [CHANGELOG.md](CHANGELOG.md) for release notes.
...
Modules live in subdirectories of [amsokol/bazel_utils](https://github.com/amsokol/bazel_utils.git). Pin each language module with `git_override` at tag `v0.2.8` and `strip_prefix` matching that directory. Language modules depend on `bazel_utils_core` (no public macros, not on the Bazel Central Registry), so add this once:

```starlark
bazel_dep(name = "bazel_utils_core", version = "0.2.8")
...
```starlark
# MODULE.bazel
bazel_dep(name = "bazel_utils_bazel", version = "0.2.8")
...
```starlark
# MODULE.bazel
bazel_dep(name = "bazel_utils_buf", version = "0.2.8")
...
```starlark
# MODULE.bazel
bazel_dep(name = "bazel_utils_protoc", version = "0.2.8")
...
```starlark
# MODULE.bazel
bazel_dep(name = "bazel_utils_go", version = "0.2.8")
...
```starlark
# MODULE.bazel
bazel_dep(name = "bazel_utils_python", version = "0.2.8")
...
```starlark
# MODULE.bazel
bazel_dep(name = "bazel_utils_rust", version = "0.2.8")
...
```starlark
# MODULE.bazel
bazel_dep(name = "bazel_utils_md", version = "0.2.8")
